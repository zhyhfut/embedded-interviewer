...
> 首次使用请先确认 Python 已正确安装：运行 `where python`，如果显示 `WindowsApps\python.exe`，请先安装真正的 Python（详见 [常见问题](#q-windows-安装脚本报错-创建虚拟环境失败-或-python-不可用)）。
```cmd
REM 1. 进入项目目录
cd embedded-interviewer
...
> 同上，先确认 `where python` 不指向 `WindowsApps\python.exe`。
```powershell
# 1. 进入项目目录
cd embedded-interviewer
...
> **注意**：Windows 10/11 自带的 `python` 命令可能是 Microsoft Store 占位程序，**不是真正的 Python**。请从官网下载安装。
...
# Windows 用户额外检查（排除 MS Store 占位程序）：
where python
# 应显示 C:\Users\...\AppData\Local\Programs\Python\Python3xx\python.exe
# 如果显示 WindowsApps\python.exe 则说明安装的不是真正的 Python
```
...
### Q: Windows 安装脚本报错 "创建虚拟环境失败" 或 "python 不可用"
A: 大概率是你的 `python` 命令指向了 **Microsoft Store 占位程序**（`WindowsApps\python.exe`），它不能运行任何 Python 代码。

**排查方法**：
```cmd
where python
```
如果显示 `C:\Users\...\AppData\Local\Microsoft\WindowsApps\python.exe`，说明是占位程序，不是真 Python。
